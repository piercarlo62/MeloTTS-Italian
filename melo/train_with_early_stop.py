...
class EarlyStopping:
    ...
    def __call__(self, val_loss, epoch, model_paths, logger=None):
        """
        ...
        Args:
            ...
            model_paths: Dict with paths to current model checkpoints
        ...
        if val_loss < self.best_loss - self.min_delta:
            ...
            if self.restore_best_weights:
                self.best_model_paths = model_paths.copy()
                if logger:
        ...
        if self.counter >= self.patience:
            ...
            if logger:
                logger.info(f"Early stopping triggered! Best loss: {self.best_loss:.6f} at epoch {self.best_epoch}")
                
        return self.early_stop
    
    def restore_best_model(self, nets, optims, logger=None):
        """Restore the best model weights"""
        if not self.restore_best_weights or not self.best_model_paths:
            return
        ...
        try:
            if 'G' in self.best_model_paths:
                utils.load_checkpoint(self.best_model_paths['G'], net_g, optim_g)
                if logger:
                    logger.info(f"Restored best generator from {self.best_model_paths['G']}")
                    
            if 'D' in self.best_model_paths:
                utils.load_checkpoint(self.best_model_paths['D'], net_d, optim_d)
                if logger:
                    logger.info(f"Restored best discriminator from {self.best_model_paths['D']}")
                    
            if 'DUR' in self.best_model_paths and net_dur_disc is not None:
                utils.load_checkpoint(self.best_model_paths['DUR'], net_dur_disc, optim_dur_disc)
                if logger:
                    logger.info(f"Restored best duration discriminator from {self.best_model_paths['DUR']}")
